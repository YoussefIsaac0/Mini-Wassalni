    ...
    if (nodes >= 2) {
        if (numOfEdges >= 1) {
            ...
            do {
                ...
                while (toInt[Location2] == 0 || Location1 == Location2) {
...
void Graph::updateGraph() { 
    cout << "\nPlease enter:\n'1' to add a location\n'2' to delete a location\n'3' to add a road\n'4' to delete a road\n'5' to update a road\n'6' to find the shortest path between two locations\n'7' to run testing mode\n'8' to return to start menu.\n>> ";
    // replaced option to terminate the program with return to be able to prompt saving
...
//Function that deletes an edge, it takes 3 arguments: the two locations connected by the edge to be deleted, and the length of the edge.
/*
void Graph::deleteEdge(int from, int to, int dist) {
    bool found = false;
    ...
    for (int i = 0; i < edges.size(); i++) {
        ...
        if (edges[i].from == from && edges[i].to == to && edges[i].distance == dist) {
        ...
        if (type == 2 && edges[i].to == from && edges[i].from == to && edges[i].distance == dist) {
            edges.erase(edges.begin() + i);
            numOfEdges--;
            i--;//~mo
        }
...
//it takes 3 arguments: the two locations connected by the edge to be checked, and the length of the edge.
/*
int Graph::findEdge(int from, int to, int dist) {
    bool found = false;
    for (int i = 0; i < edges.size(); i++) {
        if (edges[i].from == from && edges[i].to == to && edges[i].distance == dist) {
            return i;
